...
- [mermaid.json](file://themes/hugo-book/assets/mermaid.json)
</cite>

## 更新摘要
**变更内容**   
- 新增mermaid.json配置文件支持，提供更灵活的图表配置选项
- 增强主题样式系统，支持最大宽度限制和自定义主题样式
- 改进技术图表的视觉呈现效果
- 优化Mermaid渲染性能和用户体验
...
participant Config as "mermaid.json配置"
...
- 配置项
  ...
  - `sequence`: 时序图特定配置
- 使用方式
  - 放置在`themes/hugo-book/assets/mermaid.json`
  - 支持覆盖默认配置值
  - 可与主题变量协同工作
...
- 常见问题
  ...
  - 样式异常：确认主题样式是否被覆盖或冲突
  - 性能卡顿：评估图表复杂度与资源加载顺序
  - **新增** 配置错误：检查mermaid.json语法和配置项有效性
- 定位步骤
  - 打开浏览器控制台查看脚本加载与报错
  - 检查页面源码中是否存在Mermaid容器与脚本引用
  - 对比不同主题变量对图表外观的影响
  - **新增** 验证mermaid.json配置格式和路径
- 修复建议
  - 修正Mermaid语法错误
  - 调整短代码初始化逻辑以避免重复渲染
  - 清理冗余样式规则
...
## 结论
通过在Hugo Book主题中集成Mermaid，可以在文档中以声明式的方式快速生成高质量图表。合理的配置、清晰的样式管理与良好的性能策略，是保证用户体验与可维护性的关键。**新增的mermaid.json配置系统提供了更灵活的图表定制能力**，支持全局样式管理和性能优化。建议在团队内建立统一的Mermaid编码规范与审查流程，持续提升可视化质量与效率。
...
- 基本结构
  ...
  - 支持嵌套的配置分组
- 常用配置项
  - `maxWidth`: 设置图表最大宽度（像素值）
  - `theme`: 选择内置主题（default, forest, dark, neutral）
  - `securityLevel`: 安全级别（strict, relaxed, sandbox）
  - `flowchart`: 流程图特定配置
    - `htmlLabels`: 是否支持HTML标签
    - `curve`: 曲线类型（basis, linear, cardinal）
  - `sequence`: 时序图特定配置
    - `actorMargin`: 参与者边距
    - `messageAlign`: 消息对齐方式
- 配置优先级
  - 页面级配置 > mermaid.json配置 > 默认配置
  - 主题变量可覆盖部分配置值
