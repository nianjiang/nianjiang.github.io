...
A --> E["i18n/<br/>本地化文案"]
F["themes/hugo-book/<br/>主题源码"] --> G["layouts/_default/*<br/>基础布局与页面模板"]
F --> H["layouts/partials/docs/*<br/>头部/尾部/菜单/搜索等片段"]
F --> I["assets/book.scss<br/>样式入口"]
F --> J["assets/themes/_light.scss / _dark.scss<br/>明暗主题变量"]
F --> K["assets/_variables.scss<br/>全局变量"]
F --> L["assets/_custom.scss<br/>用户覆盖层"]
F --> M["assets/search.js<br/>前端搜索逻辑"]
F --> N["assets/sw-register.js / sw.js<br/>PWA 注册与服务端缓存"]
F --> O["i18n/*.yaml<br/>多语言键值"]
```
...
- 样式体系
  ...
  - themes/_light.scss / _dark.scss：明暗主题变量集。
  - _custom.scss：用户覆盖层，推荐在此处进行二次定制。
...
Dark --> Custom
Custom --> Main["应用 _main.scss 布局与组件样式"]
...
## 结论
通过理解 hugo-book 的主题结构与扩展点，你可以以最小侵入的方式实现高度个性化的站点体验。建议遵循“变量优先、片段覆盖、按需引入”的原则，在保证可维护性的同时获得良好的性能表现。
