...
## 项目结构
仓库采用典型的 Hugo 站点组织方式：
- 源码与配置位于根目录（如 config.yaml、hugo.toml）
- 内容位于 content 目录
...
### 站点配置与环境变量
- Hugo 配置：通过 config.yaml 或 hugo.toml 管理站点元数据、主题、插件等
...
- 内部依赖：站点配置（config.yaml/hugo.toml）、内容（content）、主题（themes）
...
WF --> DEPLOY["部署"]
WF --> RELEASE["发布(标签/日志)"]
HUGO --> CFG["站点配置(config.yaml/hugo.toml)"]
HUGO --> CNT["内容(content)"]
HUGO --> THM["主题(themes)"]
```
