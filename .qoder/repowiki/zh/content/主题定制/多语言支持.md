...
- 站点根级配置文件：hugo.toml、config.yaml
...
```mermaid
graph TB
A["站点根配置<br/>hugo.toml / config.yaml"] --> B["i18n 目录<br/>i18n/*.yaml"]
A --> C["内容目录<br/>content/*"]
...
- 多语言配置入口
  ...
  - 参考路径：[hugo.toml](file://hugo.toml)、[config.yaml](file://config.yaml)
- 翻译键值管理
...
## 依赖关系分析
- 配置到模板
  - hugo.toml/config.yaml 决定 supportedLanguages 与 defaultLanguage，影响模板渲染时的语言上下文。
- 模板到翻译
...
```mermaid
graph LR
CFG["站点配置<br/>hugo.toml/config.yaml"] --> LAYOUTS["模板与短代码<br/>layouts/**"]
