...
- [body.html](file://layouts/partials/docs/inject/body.html)
...
## 更新摘要
**变更内容**   
- 新增主题注入点机制章节，详细说明CSS和JavaScript的细粒度自定义方法
- 更新CSS覆盖与扩展章节，强调推荐使用注入点而非直接修改主题文件
- 新增注入点架构流程图，展示注入点在主题渲染流程中的位置
- 更新故障排查指南，包含注入点相关的常见问题解决方案
...
**更新** 新增了主题注入点机制，支持更细粒度的CSS和JavaScript自定义，推荐使用注入点而非直接修改主题文件。
...
P["layouts/partials/docs/inject/<br/>注入点目录"] --> Q["body.html<br/>页面底部注入"]
```
...
- **注入点机制**：支持在页面渲染过程中插入自定义CSS和JavaScript代码，提供更灵活的定制方式。
...
**更新** 推荐使用注入点机制进行细粒度定制，相比直接修改主题文件更加安全和灵活。
...
### 主题注入点机制（新增）
主题注入点机制提供了更细粒度的CSS和JavaScript自定义能力，推荐替代直接修改主题文件的方式。

- 注入点位置
  - `layouts/partials/docs/inject/` 目录存放各种注入点模板
  - `body.html` 允许在页面底部注入自定义HTML、CSS和JavaScript
- 使用方式
  - 在项目根目录创建对应的注入点文件
  - 注入的内容会在主题渲染时自动加载
  - 支持条件注入和动态内容
- 优势特点
  - 无需修改主题源码，升级时不会丢失自定义内容
  - 支持更精细的控制，可以针对特定页面或组件
  - 便于团队协作和维护
- 最佳实践
  - 将大型自定义样式放入独立的CSS文件
  - JavaScript代码应遵循模块化原则
  - 合理使用注入点的生命周期钩子

```mermaid
flowchart TD
A["主题渲染开始"] --> B["检查注入点目录"]
B --> C{"是否存在 inject/body.html?"}
C --> |是| D["加载注入点内容"]
C --> |否| E["跳过注入步骤"]
D --> F["注入CSS样式"]
F --> G["注入JavaScript脚本"]
...
- **注入点优化**：合理使用注入点机制，避免过度依赖直接文件修改，便于维护和升级。
...
- 移动端布局错乱
  ...
  - 验证侧边栏与内容区的堆叠逻辑。
- **注入点问题**
  - 确认注入点文件路径正确：`layouts/partials/docs/inject/`
  - 检查注入内容的语法错误和浏览器控制台报错。
  - 验证注入点是否在正确的渲染阶段执行。
...
## 结论
通过变量驱动的样式体系与清晰的主题分层，hugo-book 提供了高度可定制的视觉框架。新增的注入点机制进一步增强了定制的灵活性，使得开发者可以在不侵入源码的前提下，完成从配色到排版、从布局到动画的全面定制，同时保持良好的性能与可维护性。遵循"变量优先、语义明确、按需扩展"的原则，结合注入点机制，你可以更安全、更灵活地实现个性化的视觉风格。
...
- 主题切换
  ...
  - 如需手动切换，补充前端控制逻辑。
- **注入点定制**
  - 在 `layouts/partials/docs/inject/` 目录创建自定义注入点。
  - 使用注入点添加特定的CSS样式和JavaScript功能。
  - 避免直接修改主题文件，优先使用注入点机制。