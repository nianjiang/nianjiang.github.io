...
本文档定义在本目录生成一份新「英语学习计划」的标准方法：建议 4 周 × 每周 6 天（Day 01–24），按主题实际内容可长可短；每天一个 Topic + Video + Article + English Practice。
...
**风格参考**：[50-AI Topics.md](./50-AI%20Topics.md) —— 仅参考其表格风格、label/note 措辞与参考块写法；结构与标签以本文为准（建议 4 周、无每周基础对）。
...
- Day 编号全文件连续，两位补零（按总天数顺延；建议 01–24）
...
## 3. 主题递进（建议 4 周，按主题可长可短）

- 总体弧线：领域总览与核心概念（周 1）→ 关键机制（周 2）→ 工程与应用（周 3）→ 风险、权衡与总结（周 4）；按主题调整，保持由浅入深、末周收束于"做选择 + 总复盘"
- 每周一个主题（`## Week N — <Theme>`）；建议 4 周，按主题实际内容可长可短（如 2 周、6 周、8 周均可），Day 编号顺延；扩展的周作为深化周（原理 / 实操 / 案例按主题定），总结周永远收尾
...
**第一阶段：生成整体框架**
1. 确定周数（建议 4 周，按主题可长可短）与每周主题
2. 写入文件骨架：front matter + `## Week 1…N — <Theme>`（每周仅标题与主题，暂不填表格）
3. 构建验证（exit 0）

**第二阶段：逐周填充具体内容**
4. 逐周推进：调研本周 6 天的视频/文章 → 写入本周 6 行表格 + 参考块 → 构建验证 → 再调研下一周
5. **不要**预先批量调研全部周再统一写入
6. 全部周完成后执行第 7 节全套验证并汇报

## 9. 收尾清单

- [ ] 文件名与 front matter（weight = 目录下一个空闲号，title = 主题）
- [ ] 建议 4 周 × 6 行（按主题可长可短），Day 01–NN 连续无重复；无每周 `Video:` / `Article:` 声明行
