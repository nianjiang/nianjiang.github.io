...
## 更新摘要
**变更内容**   
- 移除了Git子模块相关的故障排除指南，因为项目已完全迁移到Go模块依赖管理
- 更新了依赖管理章节，强调Go模块的使用方式
- 简化了构建配置说明，移除子模块同步相关内容
...
- config.yaml：站点配置（控制主题行为、语言、菜单、SEO、构建选项等）
...
- 站点配置：通过 config.yaml 控制主题行为、语言、菜单、SEO、构建选项等。
...
### 构建与配置
- 配置文件：通过 config.yaml 控制站点的所有配置项，包括主题参数、语言设置、菜单结构、SEO 优化和构建选项。
...
- Go 模块管理：项目使用标准的 Go 模块进行依赖管理，无需 Git 子模块。
